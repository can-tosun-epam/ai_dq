...
```bash
conda create -n ai_dq python=3.9
conda activate ai_dq
```
...
```
This command will create your personal repository under `/Repos/<username>/ai_dq`.
...
- To trigger the release pipeline, get the current version from the `ai_dq/__init__.py` file and tag the current code version:
```
